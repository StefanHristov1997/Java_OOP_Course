...
public class ControllerImpl implements Controller {
    ...
    public String startGame() {
        Collection<Magician> aliveMagicians = magicians.getData().stream().filter(Magician::isAlive).collect(Collectors.toList());
        return region.start(aliveMagicians);
    }

    @Override
    public String report() {
        Stream<Magician> sorted = magicians.getData().stream().sorted(Comparator.comparing(Magician::getHealth)
                .thenComparing(Magician::getUsername).
                thenComparing(magician -> magician.getClass().getName()));

        return sorted.map(Magician::toString).collect(Collectors.joining(System.lineSeparator()));
    }
}
